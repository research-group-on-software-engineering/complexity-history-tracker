...
const datasets = [...classesnames].map((classname)=> {
    ...
    // const uniqueClassStart = filteredByClassname.filter(item => item.commit_date.startsWith(`2003-09`)).pop()
    ...
    for (let year = 2011; year <= YEAR_COUNT; year++) {
        
        // 2010-DEC 2020-DEC
        const uniqueClass =  filteredByClassname.filter(item => item.commit_date.startsWith(`${year}-12-`)).pop()
